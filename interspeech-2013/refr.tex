...
\begin{table}[ht]
  ...
  \hline
  \small
  \textbf{\# Training} & \textbf{\# Params} & \textbf{min/epoch} & \textbf{WER}\\
  \hline
  baseline & - & - & 32.36\\
  270k & 1.9m & 3 & 31.05\\
  1.3m & 6.8m & 10 & 30.93\\
  \hline
  \end{tabular}
  \caption{\label{tab:experiments}Results on development data for two different
  models trained with the REFR Hadoop interface.  The
  number of training examples, and the number of parameters in the trained model
  as well as the training time per epoch are shown.  In both cases, we
  distributed training using 100 shards for the distributed model representation
  (100 reducer nodes).}
\end{table}
  
Table~\ref{tab:experiments} presents two example runs of the REFR Hadoop
implementation.  We evaluated on a test set of 32k examples
from the IBM Fischer dataset\cite{fischeribm}.  We train REFR on
100-best lists generated by a baseline ASR system on Fischer data of different
sizes.
